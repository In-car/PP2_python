...
def draw_toolbar():
    ...
    text = f"Tool: {tool.capitalize()} | Color: {current_color} | [R] Rectangle [C] Circle [S] Square [T] Triangle [H] Rhombus [E] Eraser [1-4] Colors"
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.pos[1] > TOOLBAR_HEIGHT:
        ...
        elif event.type == pygame.MOUSEMOTION and drawing and tool == "eraser":
            pygame.draw.line(screen, WHITE, start_pos, event.pos, 10)
            start_pos = event.pos  
            ...
            if tool == "rectangle":
                ...
                pygame.draw.polygon(screen, current_color, [start_pos, (end_pos[0], start_pos[1]), ((start_pos[0] + end_pos[0]) // 2, end_pos[1])], 2)
            elif tool == "rhombus":
                w, h = abs(end_pos[0] - start_pos[0]), abs(end_pos[1] - start_pos[1])
                center = ((start_pos[0] + end_pos[0]) // 2, (start_pos[1] + end_pos[1]) // 2)
                points = [(center[0] - w//2, center[1]), (center[0], center[1] - h//2), (center[0] + w//2, center[1]), (center[0], center[1] + h//2)]
